"""Painel de estatisticas -- bloco Geral + 6 quebras em grid 2x3 com barras textuais."""
...
class StatsPanel(Vertical):
    """Painel de estatisticas com bloco Geral e 6 quebras."""
    ...
    def update_all_stats(
        ...
        self.query_one("#stats-direction", Static).update(_build_breakdown_block("POR DIRECAO", by_direction))
        self.query_one("#stats-strategy", Static).update(_build_breakdown_block("POR ESTRATEGIA", by_strategy))
        self.query_one("#stats-conf", Static).update(_build_breakdown_block("FAIXA CONFIANCA", by_conf))
    ...
    def _render_general(self, stats: dict, streak: tuple[str, int]) -> None:
        ...
        t.append("CONF MEDIA  ", style="cyan dim")
    ...
    def _render_recent(self, outcomes: list[str]) -> None:
        ...
        if last10:
            ...
            t.append("Ultimos 10  ", style="cyan dim")
        ...
        if last50:
            ...
            t.append("Ultimos 50  ", style="cyan dim")
            t.append(bar50, style="cyan")
            t.append(f"  {wr50:.0%}\n", style=_wr_style(wr50))

        if outcomes:
            t.append("Sequencia   ", style="cyan dim")
