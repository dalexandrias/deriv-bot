"""Painel de mercado: tabela comparativa M5×M15 + pré-análise + timing."""
...
class MarketPanel(Vertical):
    """Painel de indicadores de mercado comparativo M5×M15."""
    ...
    def _render_pre_analysis(self, pre: dict) -> None:
        ...
        table.add_row("Janela", Text(window, style="green" if window == "FAVORÁVEL" else "red"))

        bias = pre.get("m15_bias", {}).get("bias", "?")
        table.add_row("Viés M15", Text(bias, style="green" if bias == "BULLISH" else "red" if bias == "BEARISH" else "white"))

        conf = pre.get("confluence", {})
        table.add_row("Confluência", _conf_bar(conf.get("call_signals", 0), conf.get("put_signals", 0)))

        strat = pre.get("suggested_strategy", "?")
        table.add_row("Estratégia", Text(strat, style="cyan"))
    ...
    def _render_timing(self) -> None:
        ...
        if self._last_candle_time:
            try:
                ...
                t.append("Último candle M5  ", style="cyan dim")
                t.append(f"{dt_brt.strftime('%H:%M:%S')} (há {m}m{s:02d}s)\n", style="white")
            except Exception:
                t.append(f"Último candle M5  {self._last_candle_time}\n", style="white")
